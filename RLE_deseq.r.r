#lets you take system arguments
...
###########################
getwd()
#C:/Users/Joe/Desktop/big_data_rna_seq

setwd("C:/Users/Joe/Desktop/quantseq/zdf_Liver")

#leans first

name_of_csv_data_file = "Raw_gene_counts_liver_lean.csv"
...
RLE_norm_factor <- calcNormFactors(new_seq_data, method = "RLE")
RLE_norm_factor
...
#now ZDF
name_of_csv_data_file = "Raw_gene_counts_liver_ZDF.csv"
...
TMM_norm_factor <- calcNormFactors(new_seq_data, method = "TMM")

# normalize data by TMM method
TMM_normed_seqdata <- t(apply(new_seq_data, 1,function(x)x*TMM_norm_factor))
TMM_normed_seqdata <- round_df(TMM_normed_seqdata,0)

#print the organized csv file
csv_output_file_name <- paste("TMM_Normalized_",name_of_csv_data_file, sep="_")

write.csv(TMM_normed_seqdata, file = csv_output_file_name)
...
#ZDF versus Lean
name_of_csv_data_file = "Raw_gene_counts_liver.csv"
...
split <- (strsplit(colnames(new_seq_data), "_"))
split
g <- sapply(strsplit(colnames(new_seq_data), "_"), `[`, 2)
g
...
file_original = "RLE_deseq_ZDF_vs_Lean"
...
#compare whole egg vs casein across all samples (by genotype)
#ZDF versus Lean
#extract groups from colnames
split <- (strsplit(colnames(new_seq_data), "_"))
split
...
file_original = "RLE_deseq_WE_versus_CAS"
